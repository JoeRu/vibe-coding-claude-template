...
5. **If no duplicate** → create new item:
   ...
   - For features: add justification, tasks, verification section
   - For enablers: add `mapped-to-feature` attribute if parent feature ID given
   - For requirements: add justification only; note that `/translate` will decompose it

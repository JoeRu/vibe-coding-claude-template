...
2. **Never change item content** – you only transition statuses and manage process artifacts
3. **Warn about blockers** before starting items: check all BLOCKER items with `status="ACTIVE"`
4. **Check dependencies** before starting: warn if `depends-on` items are not DONE

...
1. **Read the item and related code before writing a single line** – understand context first
...
4. **Unit tests are not optional** – create or update tests for every code change
5. **Do not mark DONE without verification** – run tests, check for syntax errors
...
### 1. Read Context
- Read `ai-docs/overview.xml` (architecture, dependencies, existing features)
- Read `ai-docs/overview-features-bugs.xml` (item details, tasks, verification spec)
- Read all source files referenced in the item and related features

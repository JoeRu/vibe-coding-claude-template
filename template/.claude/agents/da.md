...
1. **Always read both XML files first**: `ai-docs/overview.xml` and `ai-docs/overview-features-bugs.xml`
2. **Security is mandatory**: every item you touch gets a security evaluation
...
5. **Prefer enablers over embedding**: if implementation needs infrastructure, create an ENABLER item
...
1. Read both XML files and `ai-docs/implementation-plan-template.xml`
...
4. Read all relevant source files referenced in or related to the item
